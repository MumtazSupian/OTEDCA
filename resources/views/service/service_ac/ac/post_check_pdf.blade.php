...
<html>
<head>
    <title>Laporan Post Check</title>
    <style>
...
<body>
    ...
    <table class="data-table bold-table">
        <tr>
            ...
            <th width="20%">Pre Check</th>
            <th width="20%">Post Check</th>
            <th width="25%">Tambahan Pemeriksaan</th>
        </tr>
        <tr>
            <td class="text-left">* High Pressure : 199.1 Psi - 227.5 Psi</td>
            <td>{{ $data->pre_high_pressure ?? '-' }} PSI</td>
            <td>{{ $data->post_high_pressure ?? '-' }} PSI</td>
            <td rowspan="4" style="vertical-align: top; text-align: left; font-weight: bold;">{{ $data->catatan_tambahan }}</td>
        </tr>
        <tr>
            <td class="text-left">> Low Pressure : 21.3 Psi - 35.5 Psi</td>
            <td>{{ $data->pre_low_pressure ?? '-' }} PSI</td>
            <td>{{ $data->post_low_pressure ?? '-' }} PSI</td>
        </tr>
        <tr>
            <td class="text-left">* Suhu Outlet : 4°C - 7°C</td>
            <td>{{ $data->pre_suhu_outlet ?? '-' }} °C</td>
            <td>{{ $data->post_suhu_outlet ?? '-' }} °C</td>
        </tr>
        <tr>
            <td class="text-left">- Wind Speed : 2.5 m/s - 4 m/s</td>
            <td>{{ $data->pre_wind_speed ?? '-' }} m/s</td>
            <td>{{ $data->post_wind_speed ?? '-' }} m/s</td>
        </tr>
    </table>
    ...
    <table class="data-table">
        <tr>
            <th width="50%">Perawatan</th>
            <th width="50%">Penggantian</th>
        </tr>
        <tr>
            <td>{{ $data->perawatan ?? '-' }}</td>
            <td>{{ $data->penggantian ?? '-' }}</td>
        </tr>
    </table>
