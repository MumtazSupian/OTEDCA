...
<div style="width: 100%; box-sizing: border-box; overflow-x: hidden; padding: 20px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #f39c12; padding-bottom: 10px; margin-bottom: 20px;">
        <div>
            <h1 style="margin: 0; font-size: 24px; color: #333;">Edit Leads</h1>
        </div>
    ...
    <div style="background: #fff; border: 1px solid #ddd; border-radius: 4px; border-top: 3px solid #d2d6de; box-shadow: 0 1px 1px rgba(0,0,0,0.1);">
        <form action="{{ url('/sales/leads/' . $cabang . '/' . ($role === 'leads' ? 'leads' : $role) . '/' . $lead->id) }}" method="POST" enctype="multipart/form-data" style="padding: 20px;">
            @csrf
            @method('PUT')
            ...
            <div style="display: flex; flex-wrap: wrap; justify-content: space-between;">
                ...
                <div style="width: 48%; box-sizing: border-box;">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="text" name="alamat" value="{{ old('alamat', $lead->alamat) }}" class="form-control" placeholder="Input Kecamatan" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;" required>
                    </div>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <select name="unit_id" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;" required>
                            <option value="">- Pilih -</option>
                            @foreach($units as $u)
                                <option value="{{ $u->id }}" {{ $lead->unit_id == $u->id ? 'selected' : '' }}>{{ $u->nama_unit }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div style="margin-bottom: 15px;">
                        ...
                        <select name="respon_id" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;" required>
                            ...
                            @foreach($respons as $r)
                                <option value="{{ $r->id }}" {{ $lead->respon_id == $r->id ? 'selected' : '' }}>{{ $r->nama_respon }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div style="width: 48%; box-sizing: border-box;">
                    <div style="margin-bottom: 15px;">
                        ...
                        <select name="status_id" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;" required>
                            ...
                            @foreach($statuses as $st)
                                <option value="{{ $st->id }}" {{ $lead->status_id == $st->id ? 'selected' : '' }}>{{ $st->nama_status }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <select name="spv_id" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;" required>
                            <option value="">- Pilih -</option>
                            @foreach($spvs as $spv)
                                <option value="{{ $spv->id }}" {{ $lead->spv_id == $spv->id ? 'selected' : '' }}>{{ $spv->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    @endif
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="hidden" name="sales_id" id="sales_id_input" value="{{ $lead->sales_id }}" required>
                        <div style="display: flex;">
                            <input type="text" id="sales_name_display" value="{{ $lead->sales->nama ?? '' }}" class="form-control" placeholder="" readonly style="flex: 1; padding: 8px; border: 1px solid #ccc; border-right: none; border-radius: 0; box-sizing: border-box; background: #eee; cursor: pointer;" onclick="openSalesModal()">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="text" name="update_1" value="{{ old('update_1', $lead->update_1) }}" class="form-control" placeholder="Input Update Leads" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;">
                    </div>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 5px; font-weight: 700; font-size: 13px;">Update 2</label>
                        <input type="text" name="update_2" value="{{ old('update_2', $lead->update_2) }}" class="form-control" placeholder="Input Update Leads" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;">
                    </div>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 5px; font-weight: 700; font-size: 13px;">Update 3</label>
                        <input type="text" name="update_3" value="{{ old('update_3', $lead->update_3) }}" class="form-control" placeholder="Input Update Leads" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 0; box-sizing: border-box;">
                    </div>
                    <div style="margin-bottom: 15px;">
