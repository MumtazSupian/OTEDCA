...
<div style="width: 100%; box-sizing: border-box; overflow-x: hidden; padding: 20px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="margin: 0; font-size: 24px; font-weight: 500; color: #333;">Status <span style="font-size: 16px; color: #777;">Status Edit</span></h2>
    ...
    <div style="background: #fff; border: 1px solid #ddd; border-radius: 4px; border-top: 3px solid #3c8dbc; box-shadow: 0 1px 1px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 10px 15px; border-bottom: 1px solid #f4f4f4; display: flex; justify-content: space-between; align-items: center;">
            <h3 style="margin: 0; font-size: 18px; color: #444;">Edit Status</h3>
            <a href="{{ url('/sales/leads/status') }}" style="background: #3c8dbc; color: #fff; padding: 6px 12px; text-decoration: none; border-radius: 3px; font-size: 12px;">
        ...
        <form action="{{ url('/sales/leads/status/' . $status->id) }}" method="POST" style="padding: 20px;">
            @csrf
            @method('PUT')
            
            <div style="max-width: 600px; margin: 0 auto;">
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold; font-size: 13px;">Nama status*</label>
                    <input type="text" name="nama_status" value="{{ $status->nama_status }}" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 3px; box-sizing: border-box;" required>
                </div>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px; font-weight: bold; font-size: 13px;">Deskripsi</label>
                    <input type="text" name="deskripsi" value="{{ $status->deskripsi }}" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 3px; box-sizing: border-box;">
                </div>
