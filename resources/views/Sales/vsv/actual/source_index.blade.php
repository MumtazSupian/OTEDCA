...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .table-custom thead tr th {
        ...
        padding: 11px 12px; border-bottom: 2px solid #f87171; vertical-align: middle;
    ...
    .table-custom tbody tr td {
        padding: 10px 12px; vertical-align: middle; border-bottom: 1px solid #f1f5f9;
    ...
    .tfoot-row td {
        ...
        padding: 11px 12px; border-top: 2px solid #f87171;
    }
    ...
    .table-scroll-wrapper { max-height: 480px; overflow-y: auto; overflow-x: auto; }
...
<div class="page-wrapper">
    ...
    <div class="page-header-wrap">
        ...
        <h1 class="page-header-title">{{ $pageTitle ?? 'Actual Source Inquiry' }}</h1>
        <p class="page-header-subtitle">{{ $subTitle ?? 'Monitoring data aktual inquiry berdasarkan sumber inquiry' }}</p>
    </div>
    ...
    <div class="filter-card">
        <form method="GET" action="{{ url()->current() }}">
            ...
            <div class="filter-row">
                <span class="filter-label">Branch Manager</span>
                <div class="input-with-icon">
                    <input type="text" id="branch_manager_input" name="branch_manager" placeholder="Pilih Branch Manager..." value="{{ request('branch_manager') }}" readonly style="background:#f8fafc; color:#0f172a; font-weight:600; cursor:pointer;">
                    <button type="button" id="btnOpenBmModal" class="input-icon-btn">🔍</button>
                </div>
                <div class="name-display" id="branch_manager_name_display">{{ $branchManagerName ?? 'Pilih Branch Manager' }}</div>
            </div>
            <div class="filter-row">
    ...
    <div class="table-card">
        <div class="table-scroll-wrapper">
            <table class="table-custom">
                <thead>
                    @if(isset($viewType) && $viewType === 'source_do_inquiry')
                        <tr>
                            <th style="width:50%;">Sumber Data</th>
                            <th class="text-center" style="width:25%;">DO</th>
                            <th class="text-center" style="width:25%;">Delivery</th>
                        </tr>
                    @else
                        <tr>
                            <th style="width:28%;">Sumber Data</th>
                            <th class="text-center">New</th>
                            <th class="text-center">Repet Order</th>
                            <th class="text-center">Prospect</th>
                            <th class="text-center">Hot Pros.</th>
                ...
                <tbody>
                    @php 
                        $gNew = 0; $gRep = 0; $gProsp = 0; $gHotP = 0;
                        $gSpk = 0; $gDo = 0; $gDeliv = 0; $gLost = 0;
                    @endphp
                    @forelse($data as $row)
                        @php
                            $gNew   += $row->total_new          ?? 0;
                            $gRep   += $row->total_repeat_order ?? 0;
                            $gProsp += $row->total_prospect     ?? 0;
                            $gHotP  += $row->total_hot_prospect ?? 0;
                            $gSpk   += $row->total_spk          ?? 0;
                            $gDo    += $row->total_do           ?? 0;
                            $gDeliv += $row->total_delivery     ?? 0;
                            $gLost  += $row->total_lost         ?? 0;
                        @endphp
                        <tr>
                            <td class="td-type">{{ $row->SumberData }}</td>
                            @if(isset($viewType) && $viewType === 'source_do_inquiry')
                                <td class="td-num">{{ number_format($row->total_do ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_delivery ?? 0) }}</td>
                            @else
                                <td class="td-num">{{ number_format($row->total_new ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_repeat_order ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_prospect ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_hot_prospect ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_spk ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_do ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_delivery ?? 0) }}</td>
                                <td class="td-num">{{ number_format($row->total_lost ?? 0) }}</td>
                            @endif
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ (isset($viewType) && $viewType === 'source_do_inquiry') ? 3 : 9 }}">
                ...
                <tfoot>
                    <tr class="tfoot-row">
                        ...
                        @else
                            <td class="text-center">{{ number_format($gNew) }}</td>
                            <td class="text-center">{{ number_format($gRep) }}</td>
                            <td class="text-center">{{ number_format($gProsp) }}</td>
                            <td class="text-center">{{ number_format($gHotP) }}</td>
                            <td class="text-center">{{ number_format($gSpk) }}</td>
                            <td class="text-center">{{ number_format($gDo) }}</td>
                            <td class="text-center">{{ number_format($gDeliv) }}</td>
                            <td class="text-center">{{ number_format($gLost) }}</td>
                        @endif
                    </tr>
                </tfoot>
                @endif
                ...
                <div style="max-height: 250px; overflow-y: auto;">
                    <table style="width:100%; border-collapse:collapse; font-size:12px;">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="3" style="padding:8px; text-align:center; color:#666;">Pilih Branch Manager dulu untuk menampilkan Sales Head</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
