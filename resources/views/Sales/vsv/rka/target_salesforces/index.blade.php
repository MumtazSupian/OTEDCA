...
@section('content')
    <div style="padding: 20px; max-width: 1300px; margin: 0 auto;">
        ...
        <p style="text-align:center; color: #64748b; margin-bottom:20px; font-size: 14px;">Manajemen target produktivitas berdasarkan grading sales</p>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; padding: 0 10px;">
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <a href="{{ route('rka.target-salesforces.index') }}"
                ...
                <a href="{{ route('rka.target-salesforces.pdf') }}" 
                ...
                <a href="{{ route('rka.target-salesforces.excel') }}" 
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead style="background:#fee2e2; color:#991b1b;">
                    <tr style="border-bottom: 2px solid #f87171;">
                        <th style="border: 1px solid #cbd5e1; padding: 12px; text-align: left;">GRADING SALESFORCE</th>
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr style="background:{{ $loop->iteration % 2 == 0 ? '#f7faff' : '#ffffff' }}; border-bottom: 1px solid var(--border-color, #e2e8f0);">
                            <td style="border: 1px solid #cbd5e1; font-weight:600; text-align:left; padding-left:12px;">{{ strtoupper($row->grading) }}</td>
                            ...
                            <td style="border: 1px solid #cbd5e1; padding: 6px;">
                                <div style="display: flex; gap: 4px; justify-content: center;">
                                    ...
                                    <form action="{{ route('rka.target-salesforces.destroy', $row->id) }}" 
                                          method="POST" 
                                    ...
                                    <button type="button" onclick="confirmDelete('{{ $row->id }}', '{{ $row->grading }}')"
    ...
    <script>
        function confirmDelete(id, grading) {
            Swal.fire({
                title: 'Hapus Grading ' + grading.toUpperCase() + '?',
                text: "Data target salesforce ini akan dihapus secara permanen!",
                icon: 'warning',
