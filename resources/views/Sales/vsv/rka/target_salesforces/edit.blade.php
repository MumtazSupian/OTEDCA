    ...
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        <div style="text-align: center; margin-bottom: 30px;">
            <h2 style="font-weight: 800; color: #1e293b !important; text-transform: uppercase; margin: 0;">
                EDIT TARGET SALESFORCE
            </h2>
            <div style="width: 50px; height: 4px; background: #dc2626; margin: 10px auto; border-radius: 10px;"></div>
            <p style="color: #64748b; font-size: 14px;">Perbarui standar target untuk grading <strong>{{ $row->grading }}</strong></p>
        </div>
        ...
        <div style="background: white; width: 100%; box-sizing: border-box; max-width: 700px; padding: 35px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            <form id="editSalesforceForm" action="{{ route('rka.target-salesforces.update', $row->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div style="display: grid; width: 100%; box-sizing: border-box; box-sizing: border-box;  grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 10px;">
                    <div>
                        ...
                        <select name="grading" required style="width: 100%; box-sizing: border-box; padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px; color: #4a5568;">
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun" value="{{ $row->tahun }}" required style="width: 100%; box-sizing: border-box; padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px; color: #4a5568;">
                    </div>
                </div>
                ...
                <div style="display: grid; width: 100%; box-sizing: border-box; box-sizing: border-box;  grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    @foreach (['jan', 'feb', 'mar', 'apr', 'mei', 'jun', 'jul', 'agu', 'sep', 'okt', 'nov', 'des'] as $m)
                        <div style="background: #f0f9ff; padding: 10px; border-radius: 12px; border: 1px solid #bee3f8;">
                            ...
                            <input type="number" name="{{ $m }}" value="{{ $row->$m }}" step="0.1" min="0" style="width: 100%; box-sizing: border-box; padding: 8px; border: 1px solid #90cdf4; border-radius: 8px; font-size: 14px; text-align: center;">
                        </div>
                    @endforeach
                </div>
    ...
    <script>
        function confirmUpdate() {
            Swal.fire({
                ...
                text: "Target Salesforce akan diperbarui sesuai data di atas.",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    document.getElementById('editSalesforceForm').submit();
                }
            });
        }
    </script>
