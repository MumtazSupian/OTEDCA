...
@section('content')
    <div style="padding: 20px; max-width: 1400px; margin: 0 auto;">
        ...
        <p style="text-align:center; color: #64748b; margin-bottom:20px; font-size: 14px;">Manajemen data aktual SPK penjualan unit per tipe mobil</p>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; padding: 0 10px;">
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <a href="{{ route('current.actual-spk-by-type.pdf') }}"
                ...
                <a href="{{ route('current.actual-spk-by-type.excel') }}"
        ...
        <div style="background:#fff; padding:15px; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.3); overflow-x:auto;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr style="background:{{ $loop->iteration % 2 == 0 ? '#f7faff' : '#ffffff' }}; border-bottom: 1px solid var(--border-color, #e2e8f0);">
                            <td style="border: 1px solid #cbd5e1; font-weight:600; text-align:left; padding-left:12px;">{{ $row->type_unit }}</td>
                            ...
                            @if($isNotSH)
                                <td style="border: 1px solid #cbd5e1; font-style:italic;">{{ $row->user->name ?? 'Sistem' }}</td>
                            @endif
                ...
                <tfoot style="background:#fef2f2; color:#7f1d1d; font-weight:800; font-weight:bold;">
                    <tr>
                        <td colspan="{{ $isNotSH ? 5 : 4 }}" style="border: 1px solid #cbd5e1; padding: 10px; text-align: center;">GRAND TOTAL</td>
                        @foreach ($months as $m)
