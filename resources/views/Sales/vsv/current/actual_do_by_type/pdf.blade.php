...
<html>
<head>
    <title>Laporan Actual DO By Type</title>
    <style>
...
<body>
    ...
    @php
        // Pastikan variabel $months tersedia atau didefinisikan secara manual jika tidak dilempar dari Controller
        $months = ['jan', 'feb', 'mar', 'apr', 'mei', 'jun', 'jul', 'agu', 'sep', 'okt', 'nov', 'des'];
        
        // Perhitungan Grand Total jika belum dipassing dari Controller
        $grandTotals = [];
        foreach ($months as $m) {
    ...
    <table>
        <thead>
            ...
            <tr>
                <th style="background-color: #cce5ff; font-weight: bold; text-align: center;">MOBIL TYPE</th>
        ...
        <tbody>
            @foreach($data as $row)
            <tr>
                <td style="text-align: left;">{{ $row->type_unit ?? $row->mobil_type }}</td>
