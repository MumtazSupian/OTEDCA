...
@section('content')
    <div style="padding: 20px; max-width: 1400px; margin: 0 auto;">
        ...
        <p style="text-align:center; color: #64748b; margin-bottom:20px; font-size: 14px;">Monitoring pencapaian delivery order berdasarkan grading salesforce</p>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; padding: 0 10px;">
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <a href="{{ route('current.actual-do-salesforces.pdf') }}"
                ...
                <a href="{{ route('current.actual-do-salesforces.excel') }}"
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead style="background:#fee2e2; color:#991b1b;">
                    <tr style="border-bottom: 2px solid #f87171;">
                        <th style="border: 1px solid #cbd5e1; padding: 12px; text-align: left;">GRADING</th>
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr style="background:{{ $loop->iteration % 2 == 0 ? '#f7faff' : '#ffffff' }}; border-bottom: 1px solid var(--border-color, #e2e8f0);">
                            <td style="border: 1px solid #cbd5e1; font-weight:600; text-align:left; padding-left:12px; color: #dc2626;">
                                {{ $row->grading }}
                            </td>
                            ...
                            <td style="border: 1px solid #cbd5e1; padding: 6px;">
                                <div style="display: flex; gap: 4px; justify-content: center;">
                                    ...
                                    <form action="{{ route('current.actual-do-salesforces.destroy', $row->id) }}" method="POST"
                                        id="delete-form-{{ $row->id }}" style="display:none;">
                                        @csrf @method('DELETE')
                                    </form>
                                    <button type="button"
                                        onclick="confirmDelete('{{ $row->id }}', '{{ $row->grading }}')"
                ...
                <tfoot style="background:#fef2f2; color:#7f1d1d; font-weight:800; font-weight:bold;">
                    <tr>
                        <td colspan="{{ Auth::user()->role !== 'SH' ? 4 : 3 }}" style="border: 1px solid #cbd5e1; padding: 10px; text-align: center; letter-spacing: 1px;">GRAND TOTAL</td>
                        @foreach ($months as $m)
                            <td style="border: 1px solid #cbd5e1;">{{ number_format($grandTotals[$m], 0, ',', '.') }}</td>
                        @endforeach
                        <td style="border: 1px solid #cbd5e1; background:#1565c0;">
    ...
    <script>
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                text: "Data pencapaian salesforce ini akan dihapus secara permanen!",
                icon: 'warning',
