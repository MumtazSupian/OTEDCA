...
@section('title', 'Tambah Actual DO Salesforce')
...
@section('content')
    <div style="padding: 40px 20px; display: flex; flex-direction: column; align-items: center; min-height: 100vh;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="color: #64748b; font-size: 14px;">Input data aktual delivery order berdasarkan grading salesforce per bulan</p>
        </div>
        ...
        <div style="background: white; width: 100%; box-sizing: border-box; max-width: 800px; padding: 35px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            ...
            <form id="createForm" action="{{ route('current.actual-do-salesforces.store') }}" method="POST">
                ...
                <div style="display: grid; width: 100%; box-sizing: border-box; box-sizing: border-box;  grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    <div>
                        <label style="display: block; font-weight: 700; color: #2d3748; margin-bottom: 8px; font-size: 12px; text-transform: uppercase;">Grading Salesforce</label>
                        <select name="grading" id="grading" required style="width: 100%; box-sizing: border-box; padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px; color: #4a5568; background: white; cursor: pointer;">
                    ...
                    @foreach($months as $key => $label)
                        <div style="background: #f0f9ff; padding: 10px; border-radius: 12px; border: 1px solid #bee3f8; text-align: center;">
                            ...
                            <input type="number" name="{{ $key }}" min="0" value="0" 
                                style="width: 100%; box-sizing: border-box; padding: 8px; border: 1px solid #90cdf4; border-radius: 8px; font-size: 14px; text-align: center; color: #2d3748; background: white;">
                        </div>
                    @endforeach
                </div>
