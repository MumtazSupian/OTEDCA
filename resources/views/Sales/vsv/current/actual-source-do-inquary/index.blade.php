...
@section('title', 'Actual Source DO Inquiry')
...
@section('content')
    <div style="padding: 20px; max-width: 1400px; margin: 0 auto;">
        <h2
            ...
            📈 ACTUAL SOURCE DO INQUIRY
        </h2>
        <p style="text-align:center; color: #64748b; margin-bottom:20px; font-size: 14px;">Monitoring data aktual Delivery Order berdasarkan sumber inquiry</p>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; padding: 0 10px;">
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <a href="{{ route('current.actual-source-do-inquary.pdf') }}"
                ...
                <a href="{{ route('current.actual-source-do-inquary.excel') }}"
            ...
            <a href="{{ route('current.actual-source-do-inquary.create') }}"
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead style="background:#fee2e2; color:#991b1b;">
                    <tr style="border-bottom: 2px solid #f87171;">
                        ...
                        <th style="border: 1px solid #cbd5e1; width: 100px;">CABANG</th>
                        @if(Auth::user()->role !== 'SH')
                            <th style="border: 1px solid #cbd5e1; width: 120px;">NAMA PENGINPUT</th>
                        @endif
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr style="background:{{ $loop->iteration % 2 == 0 ? '#f7faff' : '#ffffff' }}; border-bottom: 1px solid var(--border-color, #e2e8f0);">
                            <td style="border: 1px solid #cbd5e1; font-weight:600; text-align:left; padding-left:12px;">{{ $row->source_inquary }}</td>
                            ...
                            @endforeach
                            <td style="border: 1px solid #cbd5e1; font-weight: 800; background:#fef2f2;">{{ number_format($row->total, 0, ',', '.') }}</td>
                            <td style="border: 1px solid #cbd5e1; padding: 6px;">
                                <div style="display: flex; gap: 4px; justify-content: center;">
                                    <a href="{{ route('current.actual-source-do-inquary.edit', $row->id) }}"
                                    ...
                                    <form action="{{ route('current.actual-source-do-inquary.destroy', $row->id) }}" method="POST"
                                        id="delete-form-{{ $row->id }}" style="display:none;">
                                        @csrf @method('DELETE')
                                    </form>
                                    <button type="button"
                                        onclick="confirmDelete('{{ $row->id }}', '{{ $row->source_inquary }}')"
    ...
    <script>
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                text: "Data aktual source ini akan dihapus secara permanen!",
                icon: 'warning',
