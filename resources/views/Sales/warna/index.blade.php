...
<div class="page-header">
    <div>
        <h1 class="page-title">Data Warna</h1>
        <p class="page-subtitle">Kelola daftar Warna dengan tampilan yang lebih rapi dan responsif.</p>
    </div>
    <a href="{{ route('admin.warnas.create') }}" class="btn-primary">Buat Warna</a>
</div>
...
<div style="margin-bottom:18px; display:flex; flex-wrap:wrap; gap:12px; align-items:center; justify-content:space-between;">
    <form method="GET" action="{{ route('admin.warnas.index') }}" style="display:flex; gap:8px; align-items:center; flex:1; min-width:260px;">
    ...
    @if(!empty($search))
        <a href="{{ route('admin.warnas.index') }}" style="color:#0f172a; text-decoration:none; font-weight:600;">Reset</a>
    @endif
</div>
            ...
            <tbody>
                @forelse($items as $i => $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.warnas.edit', $item) }}" class="btn-primary">Edit</a>
                            <form action="{{ route('admin.warnas.destroy', $item) }}" method="POST" style="display:inline-flex; margin-left:8px;">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" style="text-align:center; padding:18px 12px; color: #000; background: #f5f5f5;">
                            Tidak ada data Warna untuk ditampilkan.
                        </td>
                    </tr>
                @endempty
            </tbody>
