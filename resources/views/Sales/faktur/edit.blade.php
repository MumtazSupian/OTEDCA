...
@section('title', 'Edit Data Faktur Polisi')
    ...
    <div class="form-card-faktur">
        <div class="form-card-header">
            ...
            <div class="form-title-area">
                <h2>Edit Data Faktur Polisi</h2>
                <p>Perbarui informasi jumlah Faktur Polisi kendaraan.</p>
            </div>
        </div>

        <form action="{{ route('sales.faktur.update', $item->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="form-group-custom">
                    ...
                    <select name="tipe_kendaraan" id="tipe_kendaraan" class="input-custom" required>
                        @foreach($tipeKendaraanList as $tipe)
                            <option value="{{ $tipe }}" {{ old('tipe_kendaraan', $item->tipe_kendaraan) == $tipe ? 'selected' : '' }}>
                                {{ $tipe }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group-custom">
                    ...
                    <select name="cabang" id="cabang" class="input-custom" required>
                        @foreach($availableBranches as $cb)
                            <option value="{{ $cb }}" {{ old('cabang', $item->cabang) == $cb ? 'selected' : '' }}>{{ $cb }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group-custom">
                    ...
                    <select name="bulan" id="bulan" class="input-custom" required>
                        @foreach($bulanMap as $num => $b)
                            <option value="{{ $b }}" {{ old('bulan', $item->bulan) == $b ? 'selected' : '' }}>
            ...
            <div class="form-group-custom">
                ...
                <input type="number" name="jumlah" id="jumlah" min="0" value="{{ old('jumlah', $item->jumlah) }}" class="input-custom" placeholder="Contoh: 15" required style="font-size: 16px; font-weight: 700;">
            ...
            <div class="form-group-custom">
                ...
                <textarea name="keterangan" id="keterangan" rows="3" class="input-custom" placeholder="Tambahkan catatan khusus bila ada...">{{ old('keterangan', $item->keterangan) }}</textarea>
            </div>

            <div class="form-actions">
                <a href="{{ route('sales.faktur.index') }}" class="btn-cancel">Batal</a>
                <button type="submit" class="btn-submit">
                    Perbarui Data Faktur
                </button>
            </div>
        </form>
    </div>
