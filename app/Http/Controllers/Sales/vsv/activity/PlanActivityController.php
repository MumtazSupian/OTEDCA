...
class PlanActivityController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Default Plan: Bulan Berjalan (M)
        $currMonthNum = (int)$request->get('bulan', (int)date('n'));
        $currYear = (int)$request->get('tahun', (int)date('Y'));

        $performanceData = $this->calculatePerformanceData(PlanActivity::class, $selectedCabang, $currMonthNum, $currYear);
        ...
        return view('sales.vsv.activity.plan.index', array_merge(
    ...
    private function calculatePerformanceData($activityModelClass, $selectedCabang, $currMonthNum, $currYear)
    {
        ...
        return Cache::remember($cacheKey, 300, function() use ($selectedCabang, $currMonthNum, $currYear, $activityModelClass) {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::error("Activity Plan Performance Error: " . $e->getMessage());
            }
    ...
    public function create(Request $request)
    {
        ...
        $existingByType = PlanActivity::where('cabang', $cabang)
            ->where('tanggal', $targetDate)
            ->whereNotNull('type_unit')
            ->get()
            ->keyBy('type_unit');

        $existingByAct = PlanActivity::where('cabang', $cabang)
            ->where('tanggal', $targetDate)
            ->whereNotNull('activity')
            ->get()
            ->keyBy('activity');

        return view('sales.vsv.activity.plan.create', compact(
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('activity.plan.index', [
            'cabang' => $cabang,
            'bulan'  => $currMonthNum,
            'tahun'  => $currYear,
        ])->with('success', 'Target & Budget Plan berhasil disimpan untuk periode ' . $currMonthNum . '/' . $currYear);
    }

    public function show($id)
    {
        $activity = PlanActivity::findOrFail($id);
        return view('sales.vsv.activity.plan.show', compact('activity'));
    }

    public function edit($id)
    {
        $activity = PlanActivity::findOrFail($id);
        return view('sales.vsv.activity.plan.edit', compact('activity'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('activity.plan.index')->with('success', 'Data Activity Plan berhasil diperbarui');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('activity.plan.index', ['cabang' => $cabang])->with('success', 'Data Activity Plan berhasil dihapus');
    }
    ...
    public function exportPdf(Request $request)
    {
        ...
        return Pdf::loadView('activity.plan.pdf', compact('data', 'selectedCabang', 'currMonthNum', 'currYear'))
